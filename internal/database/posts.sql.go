...
const getPostsFrom = `-- name: GetPostsFrom :many
SELECT id, created_at, updated_at, body, user_id FROM posts
WHERE user_id = $1
ORDER BY created_at ASC
`

func (q *Queries) GetPostsFrom(ctx context.Context, userID uuid.UUID) ([]Post, error) {
	rows, err := q.db.QueryContext(ctx, getPostsFrom, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
